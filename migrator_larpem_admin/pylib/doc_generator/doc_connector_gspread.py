...
class DocConnectorGSpread:
    ...
    def has_user_write_permission(self, email):
        ...
        for perm in lst_permission:
            if perm.get("emailAddress") == email and perm.get("role") in [
                "owner",
                "writer",
            ]:
                return True
    ...
    def share_document(self, email):
        ...
        self._g_file.share(
            email, "user", "writer", notify=True, email_message=msg
        )
        return True
    ...
    def generate_doc(self):
        ...
        for sheet_info in self._info_sheet:
            ...
            if sheet_type not in list(DocType):
                self._error = (
                    "Internal error, not supported definition of type %s"
                    % sheet_type
                )
                print(self._error, file=sys.stderr)
                return False
            ...
            for sheet in worksheet_list:
                ...
                self._error = "Sheet '%s' not exist. Existing sheet: %s" % (
                    sheet_name,
                    lst_str_worksheet,
                )
                print(self._error, file=sys.stderr)
            ...
            if has_error or sheet_type.get_header() != header_row:
                self._error = (
                    "Header of sheet %s is %s, and expected is %s"
            ...
            if len(all_values) < 2:
                ...
                if cb:
                    ...
                    self._error = (
                        "Internal error, cannot find method to parse the sheet"
                        " with type %s." % sheet_type
                    )
                    print(self._error, file=sys.stderr)
                    return False
                ...
                adapted_sheet_name = (
                    sheet_name if not is_form_admin else "admin_" + sheet_name
                )
    ...
    def _build_schema_point(self, dct_doc):
        ...
        status = self._extract_point_from_schema(
            dct_schema_user_point, dct_doc.get("schema_user")
        )
        if status is None:
            return
        status = self._extract_point_from_schema(
            dct_schema_char_point, dct_doc.get("schema_char")
        )
        if status is None:
            return
        return True
    ...
    def _parse_sheet_type_schema(self, sheet_info, doc_sheet_name, all_values):
        ...
        for lst_item in lst_line:
            ...
            if not level.isdigit():
                msg = (
                    "The case Level need to be an integer, receive %s" % level
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
                    doc_sheet_name,
                    msg,
                )
            ...
            if impression_index:
                if not impression_index.isdigit():
                    msg = (
                        "The case ImpressionIndex need to be an integer,"
                        " receive %s" % level
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
            ...
            if min_length:
                if not min_length.isdigit():
                    msg = (
                        "The case MinLength need to be an integer, receive %s"
                        % level
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
            ...
            if min_items:
                if not min_items.isdigit():
                    msg = (
                        "The case MinItems need to be an integer, receive %s"
                        % level
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
            ...
            if max_items:
                if not max_items.isdigit():
                    msg = (
                        "The case MaxItems need to be an integer, receive %s"
                        % level
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
            ...
            if point:
                dct_point_v = self._transform_point(
                    line_number, doc_sheet_name, point
                )
            ...
            if dct_value is None:
                ...
                if diff_level > 1:
                    msg = (
                        "Cannot increase more than 1 level, but can downgrade"
                        " more than 1."
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
                    ...
                    if last_object_type == "object":
                        if "properties" not in last_object:
                            msg = (
                                "Need to be a child property of last line %i,"
                                " object type. Need to be a level %i."
                                % (line_number - 1, nb_level)
                            )
                            self._error = "L.%s S.%s: %s" % (
                                line_number,
                                doc_sheet_name,
                        ...
                        if "items" not in last_object:
                            msg = (
                                "Need to be a child item of last line %i,"
                                " array type. Need to be a level %i."
                                % (line_number - 1, nb_level)
                            )
                            self._error = "L.%s S.%s: %s" % (
                                line_number,
    ...
    def _parse_sheet_type_form(self, sheet_info, doc_sheet_name, all_values):
        ...
        for lst_item in lst_line:
            ...
            read_only_player = bool(
                read_only_player == "TRUE" or read_only_player == "VRAI"
            )
            ...
            if options:
                ...
                for option in lst_option:
                    ...
                    if count_separator > 1:
                        msg = (
                            "Need 1 ':' in options to separate key and value."
                        )
                        self._error = "L.%s S.%s: %s" % (
                            line_number,
                            doc_sheet_name,
    ...
    def _parse_sheet_type_point(self, sheet_info, doc_sheet_name, all_values):
        ...
        for row in lst_value:
            ...
            if not level:
                ...
                if not (0 < level <= 5):
                    msg = (
                        "The field level need to be an integer 1 to 5. Got %s"
                        % level
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
                ...
                msg = (
                    "The field level need to be an integer 1 to 5. Type String"
                    " and got %s" % level
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
    ...
    def _extract_section(
        self, row, line_number, doc_sheet_name, lst_section, sheet_info
    ):
        """
        ...
        if description and bullet_description:
            msg = (
                "Cannot have a description and a bullet description on same"
                " line for H%s." % level
            )
            self._error = "L.%s S.%s: %s" % (line_number, doc_sheet_name, msg)
            print(self._error, file=sys.stderr)
        ...
        if description and second_bullet_description:
            msg = (
                "Cannot have a description and a second bullet description on"
                " same line for H%s." % level
            )
            self._error = "L.%s S.%s: %s" % (line_number, doc_sheet_name, msg)
            print(self._error, file=sys.stderr)
            return False
        ...
        if bullet_description and second_bullet_description:
            msg = (
                "Cannot have a bullet description and a second bullet"
                " description on same line for H%s." % level
            )
            self._error = "L.%s S.%s: %s" % (line_number, doc_sheet_name, msg)
            print(self._error, file=sys.stderr)
            return False
        ...
        if key:
            ...
            if "section" in section:
                msg = (
                    "Cannot add information on this section when contain sub"
                    " header on same line for H%s." % level
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
                    doc_sheet_name,
        ...
        if second_bullet_description:
            if "description" not in section:
                msg = (
                    "Cannot create second-bullet description missing"
                    " description for H%s." % level
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
                    doc_sheet_name,
            ...
            if lst_description:
                ...
                msg = (
                    "Cannot create second-bullet description when not precede"
                    " to bullet description for H%s." % level
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
                    doc_sheet_name,
            ...
            if not lst_bullet_description:
                ...
                if (
                    type(lst_second_bullet_description)
                    is not lst_second_bullet_description
                ):
                    lst_second_bullet_description = []
                    lst_bullet_description.append(
                        lst_second_bullet_description
                    )
        ...
        if under_level_color:
            ...
            if "under_level_color" in section:
                msg = (
                    "Already contain value of 'Under Level Color'for H%s."
                    % level
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
        ...
        if point and sub_key:
            dct_point = self._transform_point(
                line_number, doc_sheet_name, point
            )
    ...
    def _transform_point(self, line_number, doc_sheet_name, str_point):
        ...
        for str_single_point in lst_point:
            ...
            if str_single_point.count(":") != 1:
                msg = (
                    "Column 'Point' is wrong. Missing character ':' to"
                    " separate key with value. Point : %s" % str_point
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
                    doc_sheet_name,
            ...
            if "." in key:
                if key.count(".") > 1:
                    msg = (
                        f"The key '{key}' has more than 1 '.'. Support only 1"
                        " with attributes."
                    )
                    self._error = "L.%s S.%s: %s" % (
                        line_number,
            ...
            if key not in self._lst_doc_point:
                msg = (
                    f"The key '{key}' is not in system point, check sheet"
                    f" '{SHEET_SYSTEM_POINT}'."
                )
                self._error = "L.%s S.%s: %s" % (
                    line_number,
                    doc_sheet_name,
